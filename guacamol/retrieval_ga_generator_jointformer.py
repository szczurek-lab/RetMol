...
from inference_jointformer import MegaMolJointformer

import guac.guacamol_baseline.crossover as co
import guac.guacamol_baseline.mutate as mu
from guac.guacamol.utils.chemistry import canonicalize_list
from guac.guacamol.utils.data import remove_duplicates
from guac.guacamol.goal_directed_generator import GoalDirectedGenerator



sys.path.insert(1, project_home + '/MolBART')
from csv_data_retrieval_jointformer import collate_fn_inference
...
class RetrievalGAGeneratorJoint(GoalDirectedGenerator):
    """
    ...
    def __init__(self, project_home,
                 ...
                 batch_size, model_path, tokenizer_path, model_ckpt_itr=50000, max_mol_len=200):
        ...
        path_to_tokenizer_config = tokenizer_path
        # model stuff
        self.wf = MegaMolJointformer(model_path=model_path, path_to_tokenizer_config = path_to_tokenizer_config, model_ckpt_itr=model_ckpt_itr, decoder_max_seq_len=max_mol_len)
        # input molecules
    ...
    def generate_optimized_molecules(self,
        ...
        for itr in range(self.n_repeat):
            ...
            gens_ = self.wf.model.sample_molecules(batch_input ,sampling_alg='random_batch_jitter', jitter_std=1)
            gens_ = [gens_[self.n_trials * b:self.n_trials * (b + 1)] for b in range(len(lead))]
            gens2_ = self.wf.model.sample_molecules(batch_input, sampling_alg='random_batch_jitter', jitter_std=2)
            gens2_ = [gens2_[self.n_trials * b:self.n_trials * (b + 1)] for b in range(len(lead))]
            gens3_ = self.wf.model.sample_molecules(batch_input, sampling_alg='random_batch_jitter', jitter_std=3)
            ...
            batch_input = collate_fn_inference(batch)
            gens_greedy_ = self.wf.model.sample_molecules(batch_input, sampling_alg='greedy', jitter_std=None)
            ...
            for l_idx in range(len(mols_)):
                ...
                if all([s > 1e-3 for s in scores[l_idx]]) and len(scores[l_idx]) > 0:
                    if all_generateds_per_lead[l_idx] is not None:
                        ...
                        print(str(all([s > 0 for s in all_generated_attrs_per_lead[l_idx][:population_size]])))
